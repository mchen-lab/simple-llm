...
export default function SettingsModal({ open, onOpenChange }: SettingsModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (settingsToSave.model_names) {
        // Convert newlines back to commas for storage consistency
        // though backend handles both, commas are the canonical format
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[85dvh] flex flex-col p-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          {loading || !settings ? (
            ...
            >
              ...
              <TabsContent 
                value="parameters" 
                className="flex-1 overflow-auto p-6 m-0 space-y-6 focus-visible:outline-none"
              >
                {/* Open Router Section */}
              ...
              <TabsContent 
                value="models" 
                className="flex-1 overflow-hidden p-6 m-0 focus-visible:outline-none flex flex-col"
              >
